...
public sealed class HybridRuntime : MonoBehaviour
{
    ...
    {
        if (instance != null)
            throw new InvalidOperationException("The renderer owner is already installed.");
    ...
    private void Update()
    {
        ...
        try
        {
            // Resolve here too: the camera adapter is removed while rendering is suspended.
            ...
            {
                HybridSession.SetRenderingAllowed(renderingAllowed);
        ...
        catch (Exception error)
        {
            if (!startupFailed)
            {
                if (HybridSession.Installed && HybridSession.LastFailure == null)
                {
                    HybridSession.ReportFailure(error, "Renderer initialization or compatibility");
                }
                else
                {
        ...
        GameFrameBudget.Update(
            ...
            renderingAllowed && HybridSession.DetachedRenderingActive);
    ...
    {
        if (IntPtr.Size != 8)
            throw new PlatformNotSupportedException("The renderer requires the 64-bit game player.");
    ...
    private static void RequireFiles(params string[] paths)
    {
        ...
        {
            if (!File.Exists(path))
                throw new FileNotFoundException("A required renderer resource is missing.", path);
        }
    }
