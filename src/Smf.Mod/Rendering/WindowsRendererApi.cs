...
public sealed class WindowsRendererApi : INativeSession, IRetainedNativeSession, IPresentationTelemetry
{
    ...
    {
        if (IntPtr.Size != 8 || Environment.OSVersion.Platform != PlatformID.Win32NT || unityWindow == 0)
            throw new PlatformNotSupportedException("The Windows renderer needs a 64-bit Windows process and the Unity window handle.");
        AssertAbi();
        window = unityWindow;
        ...
        if (!QueryPerformanceFrequency(out long frequency) || frequency <= 0 || RenderEvent == IntPtr.Zero)
    ...
    public bool TryReadPresentation(out PresentationSample sample)
    {
        ...
        sample = new PresentationSample(state.Session, state.ActiveGeneration, count,
            timestamp, ClockFrequency, PresentationMetric.CompositionCompletions);
        return true;
    }
