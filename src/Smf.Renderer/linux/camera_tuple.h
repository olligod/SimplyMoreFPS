...
namespace linux_session {
    ...
    enum class draw_outcome { drawn, stale_camera, stale_geometry, failed, deferred };

    inline bool failed_activation_submission(draw_outcome outcome, bool submitted, bool cancelled) {
        return !submitted && !cancelled && outcome != draw_outcome::stale_geometry && outcome != draw_outcome::deferred;
    }
